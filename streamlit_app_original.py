...
# In streamlit_app.py - replace knowledge graph tab with:

    if st.session_state.last_response:
        with st.expander("📑 Source Relationships"):
            st.write("**Papers cited in this answer:**")

        sources = st.session_state.last_sources
        for i, source in enumerate(sources, 1):
            similarity = source.get('similarity', 0.0)
            st.write(f"{i}. {source['title']} ({similarity:.1%} relevance)")